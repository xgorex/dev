...
int main(){
   ...
   int **sysBuffA;
   sysBuffA = (int**)malloc(elements * sizeof(int*));

   for(int i =0; i<elements;i++){
      sysBuffA[i] = (int*)malloc(elements * sizeof(int));   
   ...
   int **sysBuffB;
   sysBuffB = (int**)malloc(elements * sizeof(int*));

   for(int i =0; i<elements;i++){
      sysBuffB[i] = (int*)malloc(elements * sizeof(int));
      for(int y =0; y<elements;y++){
         sysBuffB[i][y]=i+y;
      }
   }
   ...
   int **sysBuffC;
   sysBuffC = (int**)malloc(elements * sizeof(int*));
   for(int i=0; i<elements;i++)
      sysBuffC[i] = (int*)malloc(elements * sizeof(int));
 ...
 // Build (compile) the program for the device
   ...
   for(int h=0; h<elements;h++){
      for(int w=0; w<elements;w++){
         printf("A=%d, B=%d\n",sysBuffA[h][w],sysBuffB[h][w]); 
         //printf("Tablica C[%d][%d]=%d\n",h,w,sysBuffC[h][w]);
         //fprintf(mfile,"%d\n",sysBuffC[h][w]);
      }   
   }

   printf("[DBG] ...za FPRINT loop\n");
   //fclose(mfile);
